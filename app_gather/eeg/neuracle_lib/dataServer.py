...
class RingBuffer():
    def __init__(self,n_chan,n_points):
        ...
        self.buffer = np.zeros((n_chan+1, n_points))
        self.currentPtr = 0
        self.nUpdate = 0
...
class DataServerThread(Thread,):
        ...
        while self.shutdown_flag.isSet():
            ...
            for r in rs:
                ...
                try:
                    ...
                    socket_lock.release()
                    data = data.reshape(len(data) // (self.n_chan+1), self.n_chan+1)
    ...
    def parseData(self,raw):
        if 'Neuracle' in self.device: ## parse data according to Neuracle device protocol
            n = len(raw)
            event , hexData  = [], []
            hexData = raw[:n - np.mod(n, 4 * self.n_chan+4)] # unpack hex-data  in row
            self.buffer = raw[n - np.mod(n, 4 * self.n_chan+4):]
            n_item = int(len(hexData)/4/(self.n_chan+1))
            # format_str = '<' + (str(self.n_chan) + 'f' + '1I') * n_item
            format_str = '<' + (str(self.n_chan+1) + 'f') * n_item
            # print(len(hexData))
            parse_data = unpack(format_str, hexData)
        elif 'Emg8' in self.device: ## parse data according to Neuracle device protocol
            n = len(raw)
            event , hexData  = [], []
