...
var currentLevel = 0; // 初始化当前级别为0
// 随机选择左侧图片的函数
var is_Listening=0;
function randomizeLeftImage() {
        if (currentLevel < level) { // 确保不会超过设定的level
            if(currentLevel==0){
                trial_data = {
                    'Trial': question_num,
                    'type':'card',
            ...
            const imageIndex = leftImage_id[currentLevel];
            ...
            setTimeout(function () {
                ...
                currentLevel++; // 播放下一级别的图片
                randomizeLeftImage(); // 递归调用自己，以播放下一张图片
            }, 2000);
        }
        else
            question_num++;
...
function assignRandomImages() {
    var answer_tmp = Math.round(Math.random());
    answer.push(answer_tmp);
    // 清空 optionsContainer
    var optionsContainer = document.getElementById('optionsContainer');
    optionsContainer.innerHTML = '';
    var randomValue;
    if(!answer_tmp){                     //正确答案为：是
        var index = Math.floor(Math.random() * leftImage_id.length);
        randomValue = leftImage_id[index];
        var imagePath = '/static/img/img_memory/' + randomValue + '.jpg';
        var optionImage = createImageElement(imagePath, 'Option Image', 'optionImage');
        optionImage.style.opacity=0;
        optionImage.style.width = '100%'; 
        optionImage.className = "option";  
        optionImage.dataset.index = 0;  
        ...
        } while (leftImage_id.includes(randomValue));
        var imagePath = '/static/img/img_memory/' + randomValue + '.jpg';
        var optionImage = createImageElement(imagePath, 'Option Image', 'optionImage');
        optionImage.style.opacity=0;
        optionImage.style.width = '100%'; 
...
window.addEventListener('keydown', function(event) {
    const options = document.getElementsByClassName('ansButton');
    if(isPaused||is_Listening==0){
        return;
    }
    else{
    if(is_Listening)
...
function main() {
    ...
    for(let i = 0; i < level; i++){
        let imageIndex;
        do {
            imageIndex = Math.floor(Math.random() * 20) + 1; // 生成一个1到20的随机整数
        } while (leftImage_id.includes(imageIndex)); // 检查随机数是否已经在数组中
        leftImage_id.push(imageIndex); // 将随机数添加到数组中
    }
    currentLevel = 0;
...
window.onload = function() {
    main();
    setTimeout(function() {
        document.getElementsByClassName('ansButton')[currentIndex].classList.add('selected');
    }, wait_time) // 延迟执行选项高亮，确保选项都已经加载完成
};
