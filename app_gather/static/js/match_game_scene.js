...
function updateImages() {
    ...
    // 随机选择一个 id
    randomId = getRandomId(Object.keys(mergedData));
    // 随机选择左侧图片
    const leftImageIndex = Math.floor(Math.random() * mergedData[randomId].length);
    const sceneImagePath = mergedData[randomId][leftImageIndex].pic;
    const sceneImg = createImageElement(sceneImagePath, '场景', 'sceneImg');
    sceneImg.id = 'scene'
    sceneImg.setAttribute('type', mergedData[randomId][leftImageIndex].type)
    imageContainer.innerHTML = ''; // 清空左侧图片容器
    imageContainer.appendChild(sceneImg);
...
window.onload = function () {
    ...
    function handleOptionClick(event, option) {
        let check = true
        const dirct = document.getElementById("scene").getAttribute('type');
        if (loop_count >= loop) {
            // 显示重新开始按钮
            toggleListening();
        ...
        // }
        // 记录刺激结束时间
        // saveData(check, option, ans, loop_count, randomId);
        index_off_record(check, option, ans, loop_count, randomId, StimulusOnsetIndex);
...
// 处理异步函数
async function fetchOnIndex() {
    try {
        const data = await index_record('onSet');
...
async function saveData(check, option, ans, loopcount, random_Id) {
    try {
        const data = await index_record('offSet');
        console.log(data); // 使用获取的数据
        const trial_data = {
            'Trial': loopcount,
            'pic_id': random_Id,
            'StimulusOnsetIndex': StimulusOnsetIndex,
            'StimulusOffsetIndex': data.eeg_index,
            'pic_ans':document.getElementById('scene').getAttribute('type'),
            'ans': ans,
        }
...
async function index_off_record(check, option, ans, loopcount, random_Id, OnsetIndex) {
    try {
        const response = await fetch('/eeg/index/record', {
            method: 'POST',
            headers: {
